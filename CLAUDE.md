...
art/                  builds public/assets/ (never shipped)
  ...
  player.py           the King and the General + animations + IK rifle pose, reflex sights on the guns, first-person arms
                      with muzzle and sight points, grenade
...
- **Controls:** WASD to move, mouse to look (pointer lock), Space to jump, left click to shoot, right click to aim, G to throw a grenade. Hold Tab for the scoreboard; M toggles the full map.
...
- **First person:** the King's arms holding the equipped weapon (rifle, SMG or rocket launcher, each with its own hand pose), with recoil, walking bob and sway. The rifle and SMG carry a reflex sight built in `art/player.py` (`REFLEX_*`: a ring on a post over the rail, mounted at `RIFLE_REFLEX`/`SMG_REFLEX`, with a dot floating at its centre in its own `Dot` material that `render.ts` draws unlit and untonemapped so it glows). Aiming (right mouse) narrows the view and tightens the spread; with a sighted gun it also raises the gun to the eye so the dot sits centre screen inside the ring, and the crosshair goes. The script exports the ring's centre as a `Sight` point looking down the barrel; `render.ts` slides the whole viewmodel onto that line, `VIEWMODEL_SIGHT_DISTANCE` ahead of the eye. `npm run art -- player --preview` renders the sight picture as `viewmodel_*_ads.png` for tuning. Weapons without sights (`ironSights: false` in `shared/weapons.ts`) keep the hip pose.
